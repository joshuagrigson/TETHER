...
class RelationshipDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('RELATIONSHIP', style: TextStyle(letterSpacing: 2, fontSize: 13, fontWeight: FontWeight.w700)), actions: [IconButton(icon: const Icon(Icons.edit_outlined), onPressed: () => _showEditSheet(context, person))]),
  ...
  Future<void> _showEditSheet(BuildContext context, Person person) async {
    ...
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, backgroundColor: TetherColors.surface, builder: (sheetContext) => Padding(padding: EdgeInsets.fromLTRB(20, 20, 20, MediaQuery.viewInsetsOf(sheetContext).bottom + 20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('EDIT RELATIONSHIP', style: TextStyle(fontWeight: FontWeight.w800, letterSpacing: 2)), const SizedBox(height: 18), TextField(controller: controller, autofocus: true, decoration: const InputDecoration(labelText: 'Name', border: OutlineInputBorder())), const SizedBox(height: 14), FilledButton(onPressed: () { final value = controller.text.trim(); context.read<RelationshipProvider>().save(person.copyWith(name: value.isEmpty ? person.name : value)); Navigator.pop(sheetContext); }, child: const Text('SAVE CHANGES'))])));
    controller.dispose();
  }

  Future<void> _showNote(BuildContext context, Person person) async {
    final controller = TextEditingController();
    await showModalBottomSheet<void>(context: context, isScrollControlled: true, backgroundColor: TetherColors.surface, builder: (sheetContext) => Padding(padding: EdgeInsets.fromLTRB(20, 20, 20, MediaQuery.viewInsetsOf(sheetContext).bottom + 20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('CAPTURE A NOTE', style: TextStyle(fontWeight: FontWeight.w800, letterSpacing: 2)), const SizedBox(height: 18), TextField(controller: controller, autofocus: true, maxLines: 4, decoration: const InputDecoration(hintText: 'What should TETHER remember?', border: OutlineInputBorder())), const SizedBox(height: 14), FilledButton(onPressed: () { final value = controller.text.trim(); if (value.isNotEmpty) context.read<RelationshipProvider>().save(person.copyWith(notes: [value, ...person.notes])); Navigator.pop(sheetContext); }, child: const Text('SAVE NOTE'))])));
    controller.dispose();
  }
}
...
class _ProfileHero extends StatelessWidget {
  ...
  final Person person; final String health;
  @override
  Widget build(BuildContext context) {
    final color = health == 'THRIVING' ? TetherColors.neon : TetherColors.violet;
