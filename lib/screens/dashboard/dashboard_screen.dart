        ...
        child: CustomScrollView(
          slivers: [
            SliverPadding(
              ...
              sliver: SliverToBoxAdapter(child: _Header(searching: _searching, controller: _searchController, onSearch: _toggleSearch)),
            ),
            SliverPadding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              sliver: SliverToBoxAdapter(child: _HealthHero(people: provider.people)),
            ),
            SliverPadding(
              padding: const EdgeInsets.fromLTRB(20, 24, 20, 10),
              sliver: SliverToBoxAdapter(child: _FilterBar(selected: _filter, onChanged: (value) => setState(() => _filter = value))),
            ),
...
class _Header extends StatelessWidget {
  const _Header({required this.searching, required this.controller, required this.onSearch});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          _Metric(value: '$average%', label: 'HEALTH'),
          const SizedBox(width: 28),
          _Metric(value: '$thriving', label: 'THRIVING'),
          const SizedBox(width: 28),
          _Metric(value: '+${people.fold<int>(0, (sum, p) => sum + p.recentInteractions * 3)}', label: 'XP THIS WEEK'),
        ]),
      ]),
    );
  }
...
class _Metric extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Text(label, style: const TextStyle(fontSize: 9, color: TetherColors.muted, letterSpacing: 1.1)),
  ]);
}
...
class _FilterChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: ChoiceChip(label: Text(label, style: const TextStyle(fontSize: 10, letterSpacing: 1, fontWeight: FontWeight.w700)), selected: selected, onSelected: (_) => onTap()),
  );
}
